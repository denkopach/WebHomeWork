...
function getResultTask1(){
	...
	document.getElementById('task1-result').value = amount;
}
...
function getResultTask2(){
	...
	document.getElementById('task2-result').value = amount;
}

function getResultTask3(){
	document.getElementById('task3').innerHTML = '<span></span>';
	...
	span.innerHTML = stars + '</br>';
	insertAfter(span, task3Div.lastChild);
	stars = "";
	}
}
...
function getResultTask4(){
	...
	if(isValidateInt(input)){
		validationInputGood('task4');
		var result = [];
		...
		document.getElementById('task4-result').value = result[0] + ':' + result[1] + ':' + result[2];
	} else {
		validationInputErr('enter an integer', 'task4');
	}
}

function isValidateInt(input){
	var parsedInput = parseInt(input);
	if(input == '' || parsedInput != input) return false;
	return true;
}

function validationInputErr(message, id){
...
function validationInputGood(id){
...
function getResultTask5(){
	...
	if(isValidateInt(input)){
		validationInputGood('task5');
		var age = Number(input);
		var res = getStringNameForIntVal(age, ' год', ' года', ' лет');
		document.getElementById('task5-result').value = res;
	} else {
		validationInputErr('enter an integer', 'task4');
	}
}
...
function getResultTask6(){
	...
	if (!isValidDateTask6(inputFirst)){
		validationInputErr(errMsg, 'task6-first');
	} else if (!isValidDateTask6(inputSecond)){
		validationInputErr(errMsg, 'task6-second');
	} else {
		validationInputGood('task6-first');
		validationInputGood('task6-second');
		...
		if(timeFirst != 'Invalid Date' && timeSecond != 'Invalid Date'){
			var timeDifference = timeFirst - timeSecond;
			var result = [
			...
			document.getElementById('task6-result').value = 
...
function getResultTask7(){
	...
	if(date == 'Invalid Date' || !isValidDateTask7(input)) {
		validationInputErr('enter date in format\: 2014-12-31', 'task7');
	}
	validationInputGood('task7');
	var day = date.getDate();
	var month = date.getMonth()+1;
	if (day < 1 || day > 31) validationInputErr('enter date in format: 2014-12-31', 'task7');
	else if (month==2 && day>29) validationInputErr('enter date in format: 2014-12-31', 'task7');
	else if ((month==4||month==6||month==9||month==11) && day>30) validationInputErr('enter date in format: 2014-12-31', 'task4');
	...
	else validationInputErr('enter date in format: 2014-12-31', 'task7');
	...
	document.getElementById('task7-result').value = result;
...
function getResultTask8(){
	...
	if(isValidInputTask8(input)){
		...
		for(var i = 0; i < input[0]; i++){
			...
			insertAfter(task8DivNew, task8Div.lastChild);
		...
		validationInputErr('input format is: 8x8', 'task8');
	}
}
...
function getResultTask9(){
	...
	if(!isValidateInt(inputApartment)){
		validationInputErr('enter an integer', 'task9-apartment');
		return false;
	}
	validationInputGood('task9-apartment');
	var inputApartmentAmount = document.getElementById('task9-apartmentAmount-input').value;
	if(!isValidateInt(inputApartmentAmount)){
		validationInputErr('enter an integer', 'task9-apartmentAmount');
		return false;
	}
	validationInputGood('task9-apartmentAmount');
	var inputFloor = document.getElementById('task9-floors-input').value;
	if(!isValidateInt(inputFloor)){
		validationInputErr('enter an integer', 'task9-floors');
		return false;
	}
	validationInputGood('task9-floors');
	...
	var resultFloor = Math.ceil((resultEntrance % numApartmentInEntrace) / inputApartmentAmount);
	if(resultFloor == 0) resultFloor = inputFloor;
	document.getElementById('task9-result').value = resultEntrance + ' entrance, ' + resultFloor + ' floor';
}

function getResultTask10(){
	var input = document.getElementById('task10-input').value;
	if(isValidateInt(input)){
		validationInputGood('task10-input');
		var result = 0;
		for(var i = 0; i < input.length; i++){
			result += Number(input[i]);
		}
		document.getElementById('task10-result').value = result;
	} else {
		validationInputErr('enter an integer', 'task10');
	}
}
...
function getResultTask11(){
	...
	input = input.split(/,/);
	for(var i = 0; i < input.length; i++){
		input[i] = input[i].replace('http://', '').replace('https://', '');
	}
	input = input.sort();
